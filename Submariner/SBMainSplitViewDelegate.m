...
@implementation SBMainSplitViewDelegate
...
- (void)setSynchronizedSplitView:(NSSplitView *)splitview {
    if(synchronizedSplitView) {
        synchronizedSplitView = nil;
    }
...
- (void)setCollapsable:(BOOL)collapsable forViewAtIndex:(NSInteger)viewIndex {
    ...
    if (!collapsableByIndex) {
		collapsableByIndex = [[NSMutableDictionary alloc] initWithCapacity:0];
	}
    
	[collapsableByIndex setObject:[NSNumber numberWithBool:collapsable]
...
- (NSRect)splitView:(NSSplitView *)splitView additionalEffectiveRectOfDividerAtIndex:(NSInteger)dividerIndex {
    return [accessoryView convertRect:[accessoryView bounds] toView:splitView];
}

- (void)splitViewDidResizeSubviews:(NSNotification *)notification {
    @synchronized(self) {
        if(synchronizedSplitView != nil && [notification object] != synchronizedSplitView) {
                CGFloat position = [[[[notification object] subviews] objectAtIndex:0] bounds].size.width;
                [synchronizedSplitView setPosition:position ofDividerAtIndex:0];
        }
    }
}
...
//    //if(splitView != synchronizedSplitView) {
//        for(id key in [collapsableByIndex allKeys]) {
//            if([subview isEqualTo:[[splitView subviews] objectAtIndex:[key intValue]]]) {
//                return YES;
//            }
//        }
//        
...
//    //if(splitView != synchronizedSplitView) {
//        for(id key in [collapsableByIndex allKeys]) {
//            if(dividerIndex == [key intValue]-1) {
//                return YES;
//            }
//        }
//    //}
