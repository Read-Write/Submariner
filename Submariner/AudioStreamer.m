...
- (id)initWithURL:(NSURL *)aURL saveLocation:(NSString *)aSaveLocation
{
	...
	if (self != nil)
	{
		saveLocation = [aSaveLocation retain];
	}
	return self;
}
	...
	@synchronized(self)
	{
		...
		if (!CFReadStreamOpen(stream))
		{
            ...
            if (error.error != 0) {
                ...
                NSLog(@"error %d domain %ld", error.error, error.domain);
            }
	...
	eventType:(CFStreamEventType)eventType
{
	...
	else if (eventType == kCFStreamEventHasBytesAvailable)
	{
		...
		if (!audioFileStream)
		{
			...
			//
			AudioFileTypeID fileTypeHint = [AudioStreamer hintForFileExtension:[[url path] pathExtension]];

			// create an audio file stream parser
			err = AudioFileStreamOpen(self, MyPropertyListenerProc, MyPacketsProc, fileTypeHint, &audioFileStream);
			if (err) {
		...
		@synchronized(self)
		{
            ...
            // Save data
            if (saveLocation != nil && [saveLocation length] > 0){
                NSFileHandle *mp3 = [NSFileHandle fileHandleForWritingAtPath:saveLocation];
                
                [[NSNotificationCenter defaultCenter] addObserver:self
                                                         selector:@selector(fileHandleDataAvailableNotification:) 
